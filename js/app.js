...
var viewModel = function() {
  ...
  var button = document.getElementById('buttonMobile');
  ...
  this.selectedItemsFilter = ko.observableArray([""]);
  largeInfowindow = new google.maps.InfoWindow();

  this.filter = ko.observable();
  this.places = ko.observableArray(model.places);

  optionsDiv.style.display = "none"; // hide the menu
  ...
  for (var i = 0; i < locations.length; i++) {
    ...
    marker.setMap(map); // show the selecet marker
    markers.push(marker);
  ...
  this.showMoreMarker = function(selectedMarker, num) {
    for (var i = 0; i < markers.length; i++) {
      markers[i].setMap(null); // hide all the marker
    }
  ...
  // show the selected name from the list
  self.showSelectFilter = function() {
    for (var i = 0; i < markers.length; i++) {
